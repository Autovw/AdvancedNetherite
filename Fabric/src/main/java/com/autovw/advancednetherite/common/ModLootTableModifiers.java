...
public final class ModLootTableModifiers
{
    ...
    public static void modifyTables()
    {
        LootTableEvents.MODIFY.register(((key, tableBuilder, source, provider) ->
        {
            ...
            {
                LootPool.Builder pool = mobDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalPhantomDropChance(), Items.PHANTOM_MEMBRANE, 0, 2, ModItems.NETHERITE_IRON_SWORD);
                tableBuilder.withPool(pool);
            }

            if (source.isBuiltin() && id.equals(ZOMBIFIED_PIGLIN))
            {
                LootPool.Builder pool = mobDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalZombifiedPiglinDropChance(), Items.GOLD_NUGGET, 0, 3, ModItems.NETHERITE_GOLD_SWORD, ModItems.NETHERITE_DIAMOND_SWORD);
                tableBuilder.withPool(pool);
            }

            if (source.isBuiltin() && id.equals(PIGLIN))
            {
                LootPool.Builder pool = mobDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalPiglinDropChance(), Items.GOLD_INGOT, 1, 1, ModItems.NETHERITE_GOLD_SWORD, ModItems.NETHERITE_DIAMOND_SWORD);
                tableBuilder.withPool(pool);
            }

            if (source.isBuiltin() && id.equals(ENDERMAN))
            {
                LootPool.Builder pool = mobDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalEndermanDropChance(), Items.ENDER_PEARL, 0, 1, ModItems.NETHERITE_EMERALD_SWORD, ModItems.NETHERITE_DIAMOND_SWORD);
                tableBuilder.withPool(pool);
            }
            ...
            {
                LootPool.Builder pool = oreDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalRawIronDropChance(), Items.RAW_IRON, 1, 2, ModItems.NETHERITE_IRON_PICKAXE);
                tableBuilder.withPool(pool);
            }

            if (source.isBuiltin() && (id.equals(GOLD_ORE) || id.equals(DEEPSLATE_GOLD_ORE)))
            {
                LootPool.Builder pool = oreDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalRawGoldDropChance(), Items.RAW_GOLD, 1, 1, ModItems.NETHERITE_GOLD_PICKAXE);
                tableBuilder.withPool(pool);
            }
            ...
            {
                LootPool.Builder pool = oreDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalDiamondDropChance(), Items.DIAMOND, 1, 1, ModItems.NETHERITE_DIAMOND_PICKAXE);
                tableBuilder.withPool(pool);
            }

            if (source.isBuiltin() && id.equals(NETHER_GOLD_ORE))
            {
                LootPool.Builder pool = oreDropPool(registryLookup, (float) ConfigHelper.get().getServer().getAdditionalDropProperties().getAdditionalGoldNuggetDropChance(), Items.GOLD_NUGGET, 1, 3, ModItems.NETHERITE_GOLD_PICKAXE);
                tableBuilder.withPool(pool);
            }
             */
    ...
    private static LootPool.Builder mobDropPool(HolderLookup.RegistryLookup<Item> registryLookup, float dropChance, Item dropItem, int minDrop, int maxDrop, Item... tools)
    {
        ItemPredicate mainHandItemPredicate = ItemPredicate.Builder.item().of(registryLookup, tools).build();
    ...
    private static LootPool.Builder oreDropPool(HolderLookup.RegistryLookup<Item> registryLookup, float dropChance, Item dropItem, int minDrop, int maxDrop, ItemLike... tools)
    {
